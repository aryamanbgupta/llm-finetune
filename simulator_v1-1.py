...
def simulation_worker(worker_id: int, config: SimulationConfig, 
    ...
    logger.info(f"Assigned match IDs: {start_id} to {end_id-1}")
            ...
            for match_id, match in list(active_matches.items()):
                ...
                if match.is_complete():
                    try:
                        ...
                        if matches_completed % 100 == 0:
                            logger.info(f"Completed {matches_completed} matches, {len(active_matches)} active")
